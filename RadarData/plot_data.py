...
def plot_track(true_track, track, rain_or_reflectivity, normalization_min, threshold_value, new_size, output_timeframes, save=False, foldername=''):
  if save == True:
    try:
        os.mkdir(foldername)
    except OSError:
  ...
  if rain_or_reflectivity == 0:
    ...
    cmap = colors.ListedColormap(['silver', 'lavender','indigo','mediumblue','dodgerblue', 'skyblue','cyan',
                          'olivedrab','lime','greenyellow','orange','red','magenta','pink',])
    bounds = [-1,0,4,8,12,16,20,24,32,40,48,56]
    norm = colors.BoundaryNorm(bounds, cmap.N)
